...
def init():
    ...
    workspace_path = os.path.join(OUTPUT_DIR, 'orca_binary')
    namespace = feature + "_" + model_name # namespace is the dir name under output/orca_tpl/
    ...
    # data
    dataset_dir = '/home/jovyan/work/hpc4/orca_tpl/index/'
    ...
    utt2label = dict([line.split() for line in open(dataset_dir + 'whole_utt2label')])
...
def main(configs):
    while configs['epoch'] < configs['n_epochs'] + 1:
        losses_avg = train(configs)[0]
        net = train(configs)[1]
        configs['epoch'] += 1
        e = configs['epoch']
        configs['scheduler'].step(losses_avg)
        if e in [1, 3, 5] or e % 5 == 0:
            validate(configs)
...
def validate(configs):
    ...
    dev_loader = DataLoader(dev_dataset, batch_size=configs['batch_size'], num_workers=8)
    losses = AverageMeter()
    y_true = []
    y_pred = []
    with torch.no_grad():
    ...
    print('ROC AUC score on Devel {0:.1f}'.format(roc_auc * 100))
